...
function authorVisibilityHtml(authorMode) {
    ...
    const title = members ? 'Authors visible to members only' : 'Authors visible to everyone';
    ...
    return `<span class="mt-1 inline-flex ${color}" title="${title}"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">${icon}</svg></span>`;
}
...
function gateAccessHtml(info) {
    ...
    return `
        <div class="flex flex-col items-center leading-tight text-center">
            ...
            ${authorVisibilityHtml(info.authorMode)}
        </div>`;
}
